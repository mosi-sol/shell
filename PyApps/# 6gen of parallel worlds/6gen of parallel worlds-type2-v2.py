...
for generation in range(num_generations):
    # Introduce some randomness by allowing individuals to randomly change their decisions
    ...
    for individual_idx in range(population_size):
        # Calculate the weights for each decision option based on the previous generation's decisions, with some randomness
        weights = {option: 0 for option in decision_options}
        for decision in population[individual_idx]:
            weights[decision] += 1 + random.uniform(-decision_noise, decision_noise)
