...
def calculate_gdp():
    try:
        ...
        result_window.title("GDP Calculation Results")

        result_label = tk.Label(result_window, text=f"GDP per capita growth rate after accounting for burn rate: {growth_rate}%\nTax owed: ${tax_owed}")
        result_label.pack()
    except ValueError:
